...
class Home extends Component {
  constructor() {
    ...
    this.state = {
      ...
      passovers: [],
      checked:
        localStorage.getItem('subscribed_to_slack') == 'true' ? true : false,
      popup: false,
      slack_id: ''
    };
    this.updatePassoverProgressBars =
      this.updatePassoverProgressBars.bind(this);
  }
    ...
    Promise.all([
      ...
      .then(([res1, res2]) => {
        ...
        if (res2.status == 'success') {
          this.setState({
            passovers: res2.data.next_passovers,
            isLoading: false,
            mostRecentPass: res2.data.most_recent_passover
          });
          if (
            res2.data.next_passovers.length > 0 &&
            res2.data.most_recent_passover !== null
          ) {
  ...
  setSlack(is_subscribed) {
    ...
    let data = {
      ...
      subscribed_to_slack: is_subscribed
    };
      ...
      <div className={classes.root}>
        <Grid container spacing={2} alignItems="flex-end">
            ...
            {this.isAuthenticated() && (
              ...
              >
                ...
                <Switch
                  ...
                  onChange={(event) => this.handleToggleNotifications(event)}
                />
              </div>
            )}
          </Grid>
          ...
          >
            ...
            <DialogContent>
              <DialogContentText>
                To subscribe to slack notifications, enter your slack id (not
                username!).
              </DialogContentText>
              <TextField
                autoFocus
          ...
          <Grid item sm={8}>
            <Paper className="grid-containers">
              <Typography className="header-title" variant="h5">
                Recent Housekeeping Data
              </Typography>
              <HousekeepingList
                isLoading={this.state.isLoading}
                housekeeping={this.state.housekeeping}
                empty={this.state.emptyhk}
              />
            </Paper>
          </Grid>
          <Grid item sm={4}>
            <Paper className="grid-containers">
              ...
              <Passovers
                isLoading={this.state.isLoading}
                passovers={this.state.passovers}
                empty={this.state.emptypassover}
                mostRecentPass={this.state.mostRecentPass}
              />
            </Paper>
          </Grid>
        </Grid>
